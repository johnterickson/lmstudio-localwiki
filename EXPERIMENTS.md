...
- A returned `path`, not the display title, must be passed to `wiki_fetch`.

### `wiki_fetch`

```text
wiki_fetch(path, content = "intro", page = 1)
```

- `path` must be an exact path returned by `wiki_search`.
- `content` is `intro`, `full`, or `refs`.
- `page` is one-based.
- The default benchmark page size is 4,000 characters.
...
- Updated tool descriptions for exact paths, concise searches, content modes,
  and pagination.
...
- Direct provider smoke test confirmed the model-facing signatures:
  `wiki_search(query)` and `wiki_fetch(path, content, page)`.
